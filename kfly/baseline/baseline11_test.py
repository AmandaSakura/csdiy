...
OUTPUT_DIR = os.path.join(script_dir, '..', 'data', 'baseline11_output') # 从这里加载模型和预处理器
SUBMISSION_FILE_PATH = os.path.join(OUTPUT_DIR, 'submission_baseline11.csv')
...
def load_and_clean_data_for_predict(file_path): # Renamed to avoid conflict if you import
    ...
    # 处理 udmap 中的 botId 和 pluginId
    df[['botId', 'pluginId']] = df['udmap'].apply(lambda x: pd.Series(parse_udmap_helper(x)))
    df['botId'] = df['botId'].fillna(-1).astype(str) # 填充为 -1 并转换为字符串
    df['pluginId'] = df['pluginId'].fillna(-1).astype(str) # 填充为 -1 并转换为字符串
...
def feature_engineer_for_predict(df): # Renamed
    ...
    # 删除原始时间戳列和datetime对象列，以及用于中间计算的列
...
def preprocess_features_for_predict(df, numerical_features, categorical_features, scaler, label_encoders, is_train_mode=False): # Renamed
    ...
    for col in numerical_features:
        if df[col].isnull().any():
            ...
            else: # Fallback if feature not in scaler or scaler not fitted
...
if __name__ == '__main__':
    ...
    BATCH_SIZE_PREDICT = 2048 # Prediction batch size
