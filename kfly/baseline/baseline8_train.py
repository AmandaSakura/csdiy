...
def load_and_clean_data(file_path):
    ...
    df['did'] = df['did'].astype(str) # Retain did for aggregation
    return df
...
def preprocess_features(df, numerical_features, categorical_features, scaler=None, label_encoders=None, is_train_mode=True):
    ...
    if is_train_mode:
        ...
        embedding_dims_map = {} # In eval mode, this map will be built based on pre-fitted encoders
    ...
    for col in numerical_features:
        if df[col].isnull().any():
            # Use mean from training set if in eval mode, otherwise current df mean
            # If scaler is not yet fitted (is_train_mode=True and first time), calculate from df
            if not is_train_mode and scaler is not None and col in scaler.feature_names_in_:
                col_index = np.where(scaler.feature_names_in_ == col)[0][0]
                fill_value = scaler.mean_[col_index]
            else:
                fill_value = df[col].mean() # Fallback for training mode or if feature not in scaler
            df[col] = df[col].fillna(fill_value)

    if is_train_mode:
        df[numerical_features] = scaler.fit_transform(df[numerical_features])
        # Store feature names after fitting scaler
...
def select_features(df, numerical_features, categorical_features, target_col):
    ...
    importance_threshold = 0.005 # You can adjust this threshold
