...
def feature_engineer_for_predict(df): # Renamed
    ...
    df = df.drop(columns=['common_ts', 'datetime', 'did'])
...
def preprocess_features_for_predict(df, numerical_features, categorical_features, scaler, label_encoders, is_train_mode=False): # Renamed
    ...
    for col in categorical_features:
        ...
        # No need to recalculate embedding_dims_map here, just use the one loaded from config
        encoded_categorical_data[col] = df[f'{col}_encoded'].values

    for col in numerical_features:
        if df[col].isnull().any():
            # Use mean from training set if available in scaler, otherwise calculate from current df
            if scaler is not None and col in scaler.feature_names_in_:
                col_index = np.where(scaler.feature_names_in_ == col)[0][0]
                fill_value = scaler.mean_[col_index]
            else:
                fill_value = df[col].mean() # Fallback
            df[col] = df[col].fillna(fill_value)

    # Ensure that numerical_features are correctly ordered for transform
    # The scaler expects features in the same order as it was fitted
...
if __name__ == '__main__':
    ...
    BATCH_SIZE = 256 # Prediction batch size
    ...
    # 2. 加载测试数据，并保存原始DID和其在原始文件中的顺序 <--- 关键点
    ...
    # 基础清洗 (udmap处理和类型转换)
    df_test_processed[['botId', 'pluginId']] = df_test_processed['udmap'].apply(lambda x: pd.Series(parse_udmap_helper(x)))
    df_test_processed['botId'] = df_test_processed['botId'].fillna('unknown_bot').astype(str)
    df_test_processed['pluginId'] = df_test_processed['pluginId'].fillna('unknown_plugin').astype(str)
    df_test_processed = df_test_processed.drop(columns=['udmap'])

    # 应用时间戳特征工程
    df_test_processed['datetime'] = pd.to_datetime(df_test_processed['common_ts'], unit='ms')
    df_test_processed['hour'] = df_test_processed['datetime'].dt.hour.astype(str)
    df_test_processed['dayofweek'] = df_test_processed['datetime'].dt.dayofweek.astype(str)
    df_test_processed['month'] = df_test_processed['datetime'].dt.month.astype(str)
    df_test_processed['year'] = df_test_processed['datetime'].dt.year.astype(str)

    # 应用DID聚合特征工程
    print("  Calculating DID-based aggregated features for test data...")
    user_agg_df_test = df_test_processed.groupby('did').agg(
        user_total_events=('mid', 'size'),
        user_unique_mids=('mid', 'nunique'),
        user_unique_eids=('eid', 'nunique'),
        user_unique_device_brands=('device_brand', 'nunique'),
    ...
    # 合并聚合特征。注意：此时df_test_processed的行顺序可能会变化
    df_test_processed = df_test_processed.merge(user_agg_df_test, on='did', how='left')

    # 删除原始列
    df_test_processed = df_test_processed.drop(columns=['common_ts', 'datetime', 'did']) 

    # 4. 对测试数据进行预处理（transform）
    # 确保df_test_processed只包含模型需要的特征
    ...
    # 核心：使用在最开始加载时保存的 original_dids_for_submission 来构建提交DataFrame
