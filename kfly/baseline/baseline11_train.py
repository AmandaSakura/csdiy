...
OUTPUT_DIR = os.path.join(script_dir, '..', 'data', 'baseline11_output')
os.makedirs(OUTPUT_DIR, exist_ok=True)

TARGET = 'is_new_did'
RANDOM_SEED = 42
BATCH_SIZE = 512 # 根据服务器配置，可以尝试更大，但需要测试显存
NUM_EPOCHS = 50
LEARNING_RATE = 0.001
L2_REG_WEIGHT = 1e-5
...
UNSEEN_CATEGORY_PLACEHOLDER = '___UNSEEN___' # 未见类别占位符

# === 新增配置：是否使用抽样数据进行测试 ===
USE_SAMPLE_DATA = True  # 设置为 True 使用抽样数据，设置为 False 使用全部数据
SAMPLE_ROWS = 2000      # 如果使用抽样数据，读取的行数
# ============================================
...
# --- 3. 数据加载与基础清洗 ---
def load_and_clean_data(file_path, nrows=None):
    print(f"Loading data from: {file_path}{' (sampling ' + str(nrows) + ' rows)' if nrows else ''}")
    ...
    df['botId'] = df['botId'].fillna(-1).astype(str)
    df['pluginId'] = df['pluginId'].fillna(-1).astype(str)
...
def preprocess_features(df, numerical_features, categorical_features, scaler=None, label_encoders=None, is_train_mode=True):
    ...
    if is_train_mode:
        ...
        scaler.feature_names_in_ = np.array(numerical_features)
    else:
...
if __name__ == '__main__':
    ...
    # 根据配置决定是否加载全部数据或抽样数据
    nrows_to_load = SAMPLE_ROWS if USE_SAMPLE_DATA else None
    
    # 1. 加载和清洗数据
    df_full_train = load_and_clean_data(TRAIN_DATA_PATH, nrows=nrows_to_load)
    ...
    # 将 Pandas DataFrame 的 values 转换为 PyTorch 张量
    ...
    print(f"Positive samples (is_new_did=1): {int(pos_count)}, Negative samples (is_new_did=0): {int(neg_count)}, Pos_weight: {pos_weight.item():.2f}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 保存最佳模型
        if current_f1_score == early_stopper.best_score and early_stopper.counter == 0:
    ...
    feature_config = {
        ...
        'f1_threshold_prediction': F1_THRESHOLD_PREDICTION
    }
